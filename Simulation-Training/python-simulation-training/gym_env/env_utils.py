...
class State():
    ...
    def __str__(self) -> str:
        return f'State(x:{self.x},y:{self.y},z:{self.z},roll:{self.roll},pitch:{self.pitch},yaw:{self.yaw})'
    ...
    def randomize(self):    
        self.x = float(np.random.uniform(-1.5,1.5))
        self.y = float(np.random.uniform(-1.5,1.5))
        self.yaw = float(np.random.uniform(-np.pi,np.pi))

        return self

    def out_of_arena(self,i):
        self.x = 4.0
        self.y = i * 0.4
