...
def generate_and_save_images(model,path, test_sample,result2img= lambda x: x):
  predictions = model.decoder(test_sample)

  fig = plt.figure(figsize=(4, 4))

  for i in range(np.min([predictions.shape[0],16])):
    plt.subplot(4, 4, i + 1)
    plt.imshow(result2img(predictions[i, :, :, :]),cmap='gray')
    plt.axis('off')
