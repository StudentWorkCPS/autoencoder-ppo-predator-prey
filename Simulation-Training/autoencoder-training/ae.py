...
class CAE(tf.keras.Model):
    def __init__(self, latent_dim, input_shape=(64,64,1),output_filters=1,segmentation=False,architecture='default',latent_activation=None, freeze=None):
        ...
        if architecture == 'default':
            self.encoder = tf.keras.Sequential(
                [
                    ...
                    # No activation
                    tf.keras.layers.Dense(latent_dim, activation=latent_activation),
                ],name='encoder')
        ...
        if freeze is not None:
            ...
            if 'encoder' in freeze:
                self.encoder.trainable = False

            if 'decoder' in freeze:
