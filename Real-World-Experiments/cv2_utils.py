...
def draw_robot_view(normal_img,preprocced_img,predicted_img,latent,values):
    ...
    line_chart = np.zeros((64,64,3)) #lineChart(values,['value'])

    #line_chart = cv2.resize(line_chart,(64,64)) / 255
    # Draw Images
    
    img = draw_hor(normal_img/255,[preprocced_img,predicted_img,latent_plot,line_chart])
...
class CV2View():
    ...
    def draw_robot_view(self,normal_img,preprocced_img,predicted_img,latent,values):
        img = draw_robot_view(normal_img,preprocced_img,predicted_img,latent,values)
